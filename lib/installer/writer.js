import {
  ...
  readFileSync, cpSync, appendFileSync, renameSync,
} from 'fs';
...
export class Writer {
  ...
  // Registra arquivo como criado pelo Reversa
  _register(filePath) {
    const relative = filePath.replace(this.projectRoot + '/', '').replace(this.projectRoot + '\\', '');
    this.createdFiles.push(relative);
  }
  ...
  async installEntryFile(engine) {
    ...
    // Arquivo já existe — perguntar ao usuário
    ...
    if (strategy === 'merge') {
      ...
      const backupPath = destPath + '.reversa-backup';
